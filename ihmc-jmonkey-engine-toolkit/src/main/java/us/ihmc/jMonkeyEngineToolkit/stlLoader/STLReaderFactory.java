...
{
   /**
    * Create an STLReader based on the AssetInfo. Assumes the STL file is ASCII if the first
    * non-whitespace characters are "solid", and the next line contains either facet or endso else
    * assumes it is binary data.
    *
    * @see STLReader
    * @param assetInfo AssetInfo representing a valid STL file. Will determine if the underlying stream
    *                  can support mark/reset and do the appropriate amount of work.
   ...
   /**
    * Create an STLReader based on the InputStream. Assumes the STL file is ASCII if the first
    * non-whitespace characters are "solid", and the next line contains either facet or endso else
    * assumes it is binary data.
    *
    * @see STLReader
    * @deprecated Deprecated because mark/reset streams are not always supported when the .STL is in a
    *             .jar. Kept around for unit tests. Use {@link #create(com.jme3.asset.AssetInfo)}
    *             method in production code.
    * @param stream InputStream representing a valid STL file. Must support mark/reset, which is not
    *               the case when running from .jar.
    * @return Correct STL reader (binary or ASCII) based on loading heurstics
    * @throws IOException An error occured during parsing of this STL file
    */
   @Deprecated
